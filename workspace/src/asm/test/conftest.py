...
def pytest_addoption(parser):
    ...
    parser.addoption(
        '--code-address', type=int, action="store", default=0x0
    )
    parser.addoption(
        '--halt-address', type=int, action='store', default=0x10000
    )
...
def mu(request):
    ...
    mu.mem_write(request.config.option.code_address, Path(request.config.option.binary).read_bytes())
    yield mu
